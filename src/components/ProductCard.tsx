...
export default function ShopPage() {
  ...
  const handleAddToCart = (e: React.MouseEvent<HTMLButtonElement>, product: Product) => {
    ...
    // Set flying bubble initial style
    ...
    controls.start({
      x: [0, deltaX / 2, deltaX],
      y: [0, deltaY / 2 - 80, deltaY], // arc up by 80px
      scale: [1, 1.2, 0.5],
      opacity: [1, 0.7, 0],
      transition: { duration: 1, ease: 'easeInOut' },
    }).then(() => {
      setFlyingProduct(null);
      // Here you can also add to cart logic to your context/store
    });
  };
  ...
  return (
    <div className="p-6 grid grid-cols-3 gap-6">
      ...
      {flyingProduct && (
        <motion.div
          ref={flyingRef}
          initial={{ x: 0, y: 0, scale: 1, opacity: 1, position: 'fixed', top: 0, left: 0, pointerEvents: 'none', borderRadius: '50%', zIndex: 9999, overflow: 'hidden', boxShadow: '0 0 10px rgba(0,0,0,0.3)' }}
          animate={controls}
